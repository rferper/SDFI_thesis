...
if __name__ == '__main__':
    ...
    results = [['dataset', 'partition', 'n_fs', 'operator', 'sharpness', 'ntransitions']]
        ...
        for idx_fs in range(len(n_fs)):
            for idx_op in range(6):
                ...
                for idx_dat in range(len(datasets)):
                    ...
                    elif partition == 'quantiles':
                        fuzzy_dataset = fuzzy_data.FuzzyDataQuantiles(datasets[idx_dat], dataset)
                    else:
                    ...
                    results = results + [[datasets[idx_dat], partition, n_fs[idx_fs], idx_op, jumps.sharpness.mean(), len(jumps.sharpness)]]
    np.savetxt("ResultsSharpnessUniform3.csv",
               results,
